...
fun httpClient(userAgent: String = "BazelKmpResolver/1.0"): HttpClient = HttpClient {
    expectSuccess = true
            ...
            when (checksumValidation) {
                ...
                is ChecksumValidation.UsingHashDownloadedFile,
                    ...
                    val expected = when (hashResult) {
                        HashResult.NotDownloaded -> resolveChecksumHash(checksumValidation)
                            ?: error("impossible to get an `null` hash here")
...
private suspend fun HttpClient.resolveChecksumHash(checksumValidation: ChecksumValidation): String? =
    when (checksumValidation) {
...
private suspend fun HttpClient.checkIfAlreadyDownloaded(
    destination: Path, checksumValidation: ChecksumValidation
): HashResult = when (destination.exists()) {
